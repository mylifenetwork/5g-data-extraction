...
class Lane:
  ...
  def display_curvature_offset(self, frame=None, plot=False):
    ...
    if self.offset_warn:
      ...
      Lane.frame_offset += 1
      cv2.putText(image_copy,'Offset Warning! ', (int((
      5/600)*self.width), int((
      ...
      cv2.putText(image_copy,'Offset duration: '+str(round(sec_offset, 4))+' s', (int((
      5/600)*self.width), int((
      ...
      if sec_offset > sec_offset_limit:
        cv2.putText(image_copy,'You would be penalized.', (int((
        5/600)*self.width), int((
        100/338)*self.height)), cv2.FONT_HERSHEY_SIMPLEX, (float((
        0.5/600)*self.width)),(
